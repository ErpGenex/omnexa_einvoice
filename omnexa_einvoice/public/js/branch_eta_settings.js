...
function is_egypt_branch(frm) {
	if (omnexa.einvoice && omnexa.einvoice.getBranchCountryIso) {
		return omnexa.einvoice.getBranchCountryIso(frm) === "EG";
	}
	const iso = (frm.doc.country_iso || "").trim().toUpperCase();
	if (iso) {
		return iso === "EG";
	}
	const raw = String(frm.doc.country_code || "EG");
	return raw === "EG" || raw.startsWith("EG —");
}
...
frappe.ui.form.on("Branch", {
	...
	eta_receipt_environment(frm) {
		if (!is_egypt_branch(frm) || !frm.doc.eta_ereceipt_enabled) {
			return;
		}
	...
	eta_invoice_environment(frm) {
		if (!is_egypt_branch(frm) || !frm.doc.eta_einvoice_enabled) {
			return;
		}
