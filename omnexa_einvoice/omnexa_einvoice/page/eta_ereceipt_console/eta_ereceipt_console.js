frappe.pages["eta-ereceipt-console"].on_page_load = function (wrapper) {
	const page = frappe.ui.make_app_page({
		...
		single_column: true,
		icon: (locals.Page && locals.Page["eta-ereceipt-console"] && locals.Page["eta-ereceipt-console"].icon) || "receipt",
	});
	...
	const refresh_tax_test_button = async () => {
		if (omnexa.einvoice && omnexa.einvoice.refreshConsoleTaxTestButton) {
			await omnexa.einvoice.refreshConsoleTaxTestButton($testAuthBtn, {
				branch: filter_val("branch") || null,
				company: filter_val("company") || null,
			});
		}
