frappe.pages["eta-einvoice-console"].on_page_load = function (wrapper) {
	...
	const $root = $(`
		<div class="eta-einvoice-console">
			...
			<div class="mb-3 d-flex gap-2 flex-wrap align-items-center">
				...
				<button class="btn btn-default btn-sm" data-action="test-auth">${__("Test tax connection")}</button>
	...
	const $testAuthBtn = $root.find('[data-action="test-auth"]');
	const $testSigningBtn = $root.find('[data-action="test-signing-bridge"]');

	const refresh_tax_test_button = async () => {
		let spec = null;
		if (omnexa.einvoice && omnexa.einvoice.refreshConsoleTaxTestButton) {
			spec = await omnexa.einvoice.refreshConsoleTaxTestButton($testAuthBtn, {
				branch: filter_val("branch") || null,
				company: filter_val("company") || null,
			});
		}
		const showSigning = spec && spec.country_code === "EG" && !spec.needs_branch;
		$testSigningBtn.toggle(!!showSigning);
	};
	...
	const load_branches = async (company) => {
		...
		const r = await frappe.call({
			...
			args: {
				doctype: "Branch",
				filters: { company },
				fields: ["name", "branch_name", "country_code"],
				limit_page_length: 200,
			},
		});
		(r.message || []).forEach((b) => {
			const cc = (b.country_code || "EG").toUpperCase();
			const label = `${b.branch_name || b.name} (${cc})`;
			$sel.append(`<option value="${b.name}">${frappe.utils.escape_html(label)}</option>`);
		});
		await refresh_tax_test_button();
	};
	...
	$root.on(
		"change",
		'[data-filter="branch"], [data-filter="from_date"], [data-filter="to_date"], [data-filter="eta_status"]',
		async function () {
			if ($(this).data("filter") === "branch") {
				await refresh_tax_test_button();
			}
			load_queue();
		}
	...
	$root.on("click", '[data-action="test-auth"]', async () => {
		const branch = filter_val("branch") || null;
		const company = filter_val("company") || null;
		if (!branch && !company) {
			frappe.msgprint(__("Select a company or branch first."));
			return;
		}
		await omnexa.einvoice.runBranchTaxConnectionTest({
			branch,
			company,
			freeze_message: $testAuthBtn.text() + "…",
		});
	});
