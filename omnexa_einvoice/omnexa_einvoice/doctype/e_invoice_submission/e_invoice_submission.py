...
class EInvoiceSubmission(Document):
	...
	def _validate_lifecycle_controls(self):
		...
		adapter = (self.adapter_name or "").strip()
		if adapter == "einvoice_zatca":
			if self.reference_doctype != "Sales Invoice":
				frappe.throw(_("ZATCA submissions must reference Sales Invoice."), title=_("ZATCA"))
			return
		if adapter not in ("", "einvoice_eta", "einvoice_stub", "einvoice_zatca"):
			if self.reference_doctype != "Sales Invoice":
				frappe.throw(
					_("{0} submissions must reference Sales Invoice.").format(adapter),
					title=_("E-Invoice"),
				)
			return
	...
	def _merge_branch_defaults(self) -> None:
		...
		if not self.adapter_name:
			...
			if is_egypt_branch(self.branch):
				self.adapter_name = "einvoice_eta"
			else:
				self.adapter_name = resolve_tax_provider_for_branch(self.branch)["tax_provider"]
		base: dict = {}
		if self.extra_json and str(self.extra_json).strip():
			try:
		...
		if not isinstance(base, dict):
			return
		if (self.adapter_name or "").strip() != "einvoice_eta":
			return
		try:
...
def ensure_submission_for_document(doctype: str, docname: str):
	...
	from omnexa_einvoice.tax_engine.router import is_egypt_branch

	if branch and not is_egypt_branch(branch):
		frappe.throw(
			_(
				"Branch {0} is not configured for Egypt ETA. "
				"Use ZATCA Console or tax dispatch for country {1}."
			).format(
				branch,
				frappe.db.get_value("Branch", branch, "country_code") or "SA",
			),
			title=_("Egypt ETA"),
		)
	kind = "E-Invoice"
...
from omnexa_einvoice.e_invoice import usb_session as _usb

_branch_usb_pin_for_client = _usb.branch_usb_pin
_client_signing_secrets = _usb.client_signing_secrets
build_agent_session_body = _usb.build_agent_session_body
build_agent_sign_payload = _usb.build_agent_sign_payload
...
def create_usb_sign_session_for_branch_test(branch: str) -> dict:
	return _usb.create_usb_sign_session_for_branch_test(branch)
...
def get_e_invoice_signing_settings(name: str) -> dict:
	...
	branch = doc.branch or resolve_branch_for_document(source)
	settings = get_eta_invoice_branch_settings(branch)
	from omnexa_einvoice.e_invoice.auto_submit import branch_submission_mode, normalize_submission_mode

	submission_mode = normalize_submission_mode(
		settings.submission_mode or branch_submission_mode(branch)
	)
	return {
...
def sign_submission(
	...
	if is_receipt:
		...
		merged = sign_e_invoice_submission(
			doc,
			source,
			branch,
	...
	if not is_receipt:
		...
		live = maybe_enqueue_live_send_after_sign(name)
		out.update(live)
	return out
