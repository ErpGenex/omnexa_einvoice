...
"""ZATCA clearance — standard tax invoices (B2B)."""

from __future__ import annotations

from typing import Any

from omnexa_einvoice.zatca.phase2.api_client import submit_clearance_api
...
def submit_clearance(
	*,
	...
	status_code, body = submit_clearance_api(portal, payload, token, secret)
	result = parse_submission_result(status_code, body, status_field="clearanceStatus")
	...
	if result.get("ok"):
		try:
			cleared_xml = get_cleared_invoice_xml(
				body, request_payload=payload, is_simplified=False
			)
	...
	return {
		"ok": result.get("ok"),
		"clearance_status": result.get("zatca_status"),
