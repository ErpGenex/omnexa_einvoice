...
def sales_invoice_branch_country_code(doc) -> str:
	"""ISO country for the invoice branch (EG = Egypt ETA applies)."""
	branch = (doc.get("branch") or "").strip() if doc else ""
	if not branch:
		return "EG"
	try:
		from omnexa_einvoice.tax_engine.router import resolve_tax_provider_for_branch

		return (resolve_tax_provider_for_branch(branch).get("country_code") or "EG").upper()
	except Exception:
		return "EG"


def sales_invoice_is_egypt_branch(doc) -> bool:
	return sales_invoice_branch_country_code(doc) == "EG"
...
def get_eta_billing_type(doc) -> str:
	...
	if doc.doctype != "Sales Invoice":
		return ETA_BILLING_REGULAR
	if not sales_invoice_is_egypt_branch(doc):
		return ETA_BILLING_REGULAR
...
def validate_sales_invoice_eta_billing_type(doc, method=None) -> None:
	if doc.doctype != "Sales Invoice" or doc.get("is_return"):
		return
	if not sales_invoice_is_egypt_branch(doc):
		if sales_invoice_has_billing_type_field() and doc.get("eta_billing_type") not in (
			None,
			"",
			ETA_BILLING_REGULAR,
		):
			doc.eta_billing_type = ETA_BILLING_REGULAR
		return
