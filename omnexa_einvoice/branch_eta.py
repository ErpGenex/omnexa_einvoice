...
def validate_branch_eta_settings(doc, method=None) -> None:
	"""Branch.validate — separate checks for e-Receipt and e-Invoice (Egypt branches only)."""
	country = (doc.get("country_code") or "EG").strip().upper()
	if country and country != "EG":
		return
	...
	if int(doc.get("eta_einvoice_enabled") or 0):
		...
		# USB PIN is validated at Sign/Send time (usb_session / eta_invoice_signing), not on Branch save.
